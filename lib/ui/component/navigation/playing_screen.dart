...
class _PlayingScreenState extends ConsumerState<PlayingScreen>
  ...
  void initState() {
    ...
    // Đảm bảo animation ở trạng thái completed với màu ban đầu
  ...
  void didChangeDependencies() {
    ...
    // Sync với màu hiện tại khi widget được rebuild (push/pop)
    final currentColor = ref.read(ambientColorProvider);
    if (_previousColor != currentColor && mounted) {
      _previousColor = currentColor;
      _colorAnimation = ColorTween(begin: currentColor, end: currentColor)
          .animate(
            CurvedAnimation(parent: _colorController, curve: Curves.easeInOut),
          );
      _colorController.value = 1.0;
  ...
  Widget build(BuildContext context) {
    ...
    // Listen cho thay đổi màu và trigger animation
    ref.listen<Color>(ambientColorProvider, (previous, next) {
      if (previous != next && mounted) {
        // Lấy giá trị hiện tại của animation làm điểm bắt đầu
        final currentAnimatedValue = _colorAnimation.value ?? previous;

        // Chỉ animate nếu màu thực sự thay đổi
        if (currentAnimatedValue != next) {
          _previousColor = currentAnimatedValue;
          _colorAnimation = ColorTween(begin: currentAnimatedValue, end: next)
              .animate(
                CurvedAnimation(
...
// Widget riêng để tối ưu gradient background
class _AnimatedGradientBackground extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final containerColor = mixColors([
      MapEntry(surfaceColor, 0.3),
      MapEntry(animatedColor, 0.7),
    ]);
    final color1 = mixColors([
      MapEntry(containerColor, 0.8),
      MapEntry(surfaceColor, 0.2),
    ]);
    final color2 = mixColors([
      MapEntry(containerColor, 0.8),
      MapEntry(surfaceColor, 0.4),
    ]);
    final color3 = mixColors([
      MapEntry(containerColor, 0.2),
      MapEntry(surfaceColor, 0.8),
    ]);
...
class _AnimatedAppBar extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final color1 = mixColors([
      MapEntry(containerColor, 0.9),
      MapEntry(surfaceColor, 0.1),
    ]);
    final color2 = mixColors([
      MapEntry(containerColor, 0.8),
      MapEntry(surfaceColor, 0.2),
          ...
          return Transform.scale(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                ...
                boxShadow: [
                  BoxShadow(
                    color: ambientColor.withOpacity(0.3),
                    blurRadius: 30,
                    spreadRadius: 5,
                  ),
                ],
